...
IsaCoSy \ES{cite}, uses constraint-based synthesis to generate conjectures while trying to maintain completeness.
Other systems use templates as a heuristic to constrain or simplify conjecture generation \ES{cite Scheme-Based Synthesis of Inductive Theories, Template-based Theory Exploration: Discovering Properties of Functional Programs by Testing, MathsAid}. 

More recent, and especially successful, approaches rely on equality relations as a means to compress the search space.
QuickSpec \ES{cite both papers} uses observable equality (equality over a finite set of examples) as an equality relation.
